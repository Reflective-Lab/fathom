//! Fathom — SPARC CLI.
//!
//! `fathom-narrative analyse <CIK>` discovers JSON fixtures under `fixtures/`
...
use fathom_narrative_core::{Cik, RiskFactorSection};
use fathom_narrative_ingest::load_risk_factor_fixture;
use fathom_narrative_suggestors::{
...
/// Cedar policy text — checked into `policies/fathom_narrative.cedar`,
/// embedded at build time so a single-file `fathom-narrative` binary doesn't
/// need its policy on disk at runtime.
const POLICY_TEXT: &str = include_str!("../../../policies/fathom_narrative.cedar");

const FIXTURES_DIR: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../fixtures");

#[derive(Parser)]
#[command(
    name = "fathom-narrative",
...
async fn ingest_sec(
    ...
    let section = fathom_narrative_ingest::fetch_and_extract_sec(url, cik, fiscal_year)
        .await
...
async fn ingest_hf(
    ...
    let written = fathom_narrative_ingest::fetch_and_extract_hf_shard(dataset, shard, fixtures_dir)
        .await
    ...
    let result = loop {
        match step {
            ...
            RunResult::HitlPause(pause) => {
                ...
                let decision = GateDecision::approve(gate_id, "fathom-narrative:auto-approver");
...
async fn portfolio(budget: i64, fixtures_dir: &std::path::Path) -> anyhow::Result<()> {
    ...
    // PortfolioSuggestor reads `portfolio-budget:fathom-narrative:portfolio:risk-coverage`
...
fn load_all_sections(fixtures_dir: &std::path::Path) -> anyhow::Result<Vec<RiskFactorSection>> {
    let mut out = Vec::new();
...
fn analyst_decide_request(cik: &Cik) -> DecideRequest {
    DecideRequest {
        principal: PrincipalIn {
            id: "agent:fathom-narrative:analyst".into(),
            authority: AuthorityLevel::Participatory,
            domains: vec!["financial-analysis".into()],
            policy_version: None,
        },
        resource: ResourceIn {
            id: format!("flow:fathom-narrative:analyse:{}", cik.as_str()).into(),
