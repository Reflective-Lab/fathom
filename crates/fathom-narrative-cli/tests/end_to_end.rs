...
fn engine_promotes_both_drift_facts_for_apple_fy24_to_fy25() {
    let output = Command::new(env!("CARGO_BIN_EXE_fathom-narrative"))
    ...
    let identical = content["identical_count"]
        .as_u64()
        .expect("identical_count");
...
fn portfolio_tight_budget_picks_highest_value_cik() {
    ...
    assert_eq!(
        selections.len(),
        1,
        "exactly one foundation selection expected"
    );
