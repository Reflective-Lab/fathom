//! Build script for fathom-sparc-cli.
//!
...
fn main() {
    ...
    if std::env::var_os("CARGO_FEATURE_FERROX_MIP").is_some() {
        if let Ok(highs_root) = std::env::var("FERROX_HIGHS_ROOT") {
            // `rustc-link-arg-bins` applies only to bin targets — the right
            // scope, since the libhighs dylib search is a runtime concern of
            // the executable, not of the (workspace) library crates.
