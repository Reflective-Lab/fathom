...
use fathom_narrative_core::analytic::{RiskFactorDrift, RiskFactorLanguageDrift};
use std::collections::HashMap;

const PROVENANCE: &str = "fathom-narrative:portfolio_seed:v1";
const REQUEST_ID: &str = "fathom-narrative:portfolio:risk-coverage";
...
impl Suggestor for PortfolioCoverageSeedSuggestor {
    ...
    fn accepts(&self, ctx: &dyn Context) -> bool {
        ...
        counts.keys().any(|key| langs.contains_key(key))
    }
...
fn parse_drifts(
    ctx: &dyn Context,
) -> (DriftMap<RiskFactorDrift>, DriftMap<RiskFactorLanguageDrift>) {
...
mod tests {
    use super::*;
    use fathom_narrative_core::{Cik, FilingId, FormType};
    ...
    fn higher_churn_higher_value() {
        ...
        let a = req
            .items
            .iter()
            .find(|i| i.label.starts_with("A::"))
            .unwrap();
        let b = req
            .items
            .iter()
            .find(|i| i.label.starts_with("B::"))
            .unwrap();
