...
pub enum SecIngestError {
    ...
    SectionNotFound,
    #[error(
        "no extractor pattern produced a plausible heading count (15–40); best yielded {best} headings"
    )]
...
fn best_pattern(section_html: &str) -> Result<Vec<String>, SecIngestError> {
    ...
    for selector in SELECTORS {
        ...
        let candidates: Vec<String> = nodes
            ...
            .filter(|text| {
                let len = text.len();
                len >= MIN_HEADING_LEN && len <= MAX_HEADING_LEN && text.ends_with('.')
            })
            .collect();
...
mod tests {
    ...
    fn rejects_when_no_pattern_plausible() {
        ...
        assert!(matches!(
            err,
            SecIngestError::NoPlausiblePattern { best: 5 }
        ));
    }
}
